...
const mutations = {
  ...
  [types.SET_PAGE_QUERY_INFO_BY_SELECT_CUBE](stateEslinitRule, selectArray) {
    ...
    const kylinModels = [];
    // 根据用户选择cube设置查询条件
    state.initInfo.loadInfo.forEach(v => {
      if (v.projectCode === selectArray[0]) {
        kylinPorjects.push(v);
      }
    });
    kylinPorjects.forEach(v => {
      v.kylinLoadModels.forEach(v1 => {
        if (v1.modelCode === selectArray[1]) {
          kylinModels.push(v1);
        }
      });
    });
    kylinModels.forEach(v => {
      v.kylinLoadCubes.forEach(v1 => {
        if (v1.cubeCode === selectArray[2]) {
          cube = v1;
        }
      });
    });
