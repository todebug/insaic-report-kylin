<template>
    ...
    <el-form ref="form" :model="form" label-width="80px">
      ...
      <el-form-item label="度量">
        <el-checkbox :indeterminate="isIndeterminateMeasures" v-model="checkAllMeasure"
      ...
      <el-form-item label="维度">
        <el-checkbox :indeterminate="isIndeterminateDimensions" v-model="checkAllDimension"
          v-if="initInfo.dimensions && initInfo.dimensions.length && form.cube && form.cube.length" @change="selectAllDimension">全选</el-checkbox>
        <el-checkbox-group v-model="form.dimensions" @change="selectDimensionChecked">
          <el-checkbox v-if="form.cube && form.cube.length"
...
export default {
  ...
  data() {
    return {
      ...
      // 度量全选标识
      isIndeterminateMeasures: true,
      // 度量全选标识
      checkAllMeasure: false,
      // 维度全选标识
      isIndeterminateDimensions: true,
      // 维度全选标识
    ...
    'form.cube': {
      handler(curVal, oldVal) {
        if (oldVal.length && curVal[2] !== oldVal[2]) {
          ...
          this.isIndeterminateMeasures = true;
          this.checkAllMeasure = false;
          this.isIndeterminateDimensions = true;
          this.checkAllDimension = false;
          this.form.measures = [];
  ...
  methods: {
    ...
    selectAllMeasure(event) {
      if (event) {
        this.form.measures = [];
        if (this.initInfo.measures.length) {
          this.initInfo.measures.forEach(v => {
            const measure = `${v.code}&&${v.columnCode}`;
            this.form.measures.push(measure);
          });
        }
      } else {
        this.form.measures = [];
      }
      this.isIndeterminateMeasures = false;
    },
    // 单独选择度量触发事件
    selectMeasureChecked(value) {
      const checkedCount = value.length;
      this.checkAllMeasure = checkedCount === this.initInfo.measures.length;
      this.isIndeterminateMeasures = checkedCount > 0
      && checkedCount < this.initInfo.measures.length;
    },
    // 维度全选操作触发事件
    selectAllDimension(event) {
      if (event) {
        this.form.dimensions = [];
        if (this.initInfo.dimensions.length) {
          this.initInfo.dimensions.forEach(v => {
            this.form.dimensions.push(v.code);
          });
        }
        if (this.initInfo.dimensionsMore.length) {
          this.initInfo.dimensionsMore.forEach(v => {
            this.form.dimensions.push(v.code);
          });
        }
      } else {
      ...
      this.isIndeterminateDimensions = false;
    },
    // 单独选择维度触发事件
    selectDimensionChecked(value) {
      const checkedCount = value.length;
      const dimensionsCount = this.initInfo.dimensions.length
      + this.initInfo.dimensionsMore.length;
      this.checkAllDimension = checkedCount === dimensionsCount;
      this.isIndeterminateDimensions = checkedCount > 0 && checkedCount < dimensionsCount;
    },
    // 级联select值改变触发
